...
const tabs: { key: TabKey; name: string; path: string }[] = [
  { key: "posts", name: "Posts", path: "/providers-home" },
  { key: "my-posts", name: "My Posts", path: "/providers-home/myposts" },
  { key: "jobs", name: "Job Offers", path: "/providers-home/jobs" },
];
      ...
      <div className="flex justify-around max-w-5xl mx-auto">
        {tabs.map((tab) => (
          ...
          >
            {tab.name}
            {active === tab.key && (
              <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-1.5 bg-[#222BDE] rounded-t-full transition-all duration-300"></span>
            )}
          </NavLink>
        ))}
      </div>
